...
trainingTypes=["CIFAR_regular1_model","CIFAR_interpretablefeaturesDroped_0.5_RandomMasking_1_model"]
...
for figure in range (5):
	...
	for j in range(0,rows):
		imagePath=path+"original_image"
		img = plt.imread(imagePath+"_"+str(CIFAR[j])+".png")
		# ax[j,0].set_title(str(CIFAR[j]),fontsize=200,fontweight='bold')
		ax[j,0].imshow(img)
		ax[j,0].axis("off")
		n+=1
	...
	for j in range(0,rows):
		...
		for x in range(2):
			...
			img = plt.imread(imagePath+"_"+str(CIFAR[j])+"2.png")
			ax[j,x+1].imshow(img)
			ax[j,x+1].axis("off")
